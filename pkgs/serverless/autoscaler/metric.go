...
func PeriodicMetric(timeInterval int) {
	for {
		...
		var replicaList []core.ReplicaSet
		err := utils.JsonUnMarshal(response, &replicaList)
		if err != nil {
		...
		for _, replica := range replicaList {
			...
			if record == nil {
				record = &Record{
					...
					Replicas:  replica.Status.RealReplicas,
				...
				//if replica.Status.RealReplicas!= replica.Status.ReadyReplicas {
