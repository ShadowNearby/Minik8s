...
func CreateVolume(volume core.PersistentVolume) (*csi.Volume, error) {
	resp, err := CsiClientInstance.ControllerClient.CreateVolume(CsiClientInstance.Context, &csi.CreateVolumeRequest{
		Name: fmt.Sprintf("%s-%s", volume.MetaData.Namespace, volume.MetaData.Name),
...
func ControllerPublishVolume(csiVolume *csi.Volume, objVolume *core.PersistentVolume, nodeId string) (*csi.ControllerPublishVolumeResponse, error) {
