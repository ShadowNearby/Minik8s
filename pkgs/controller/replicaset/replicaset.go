...
func (rsc *ReplicaSetController) createReplicas(info string) error {
	...
	err = controller.SetObject(config.ObjReplicaSet, replica.MetaData.NameSpace, replica.MetaData.Name, replica)
	return err
}
...
func (rsc *ReplicaSetController) updateReplicas(info string) error {
	...
	err = controller.SetObject(config.ObjReplicaSet, replicas[1].MetaData.NameSpace, replicas[1].MetaData.Name, replicas[1])
	return err
}
...
func (rsc *ReplicaSetController) manageUpdateReplicas(oldRs *core.ReplicaSet, newRs *core.ReplicaSet) error {
	if oldRs.MetaData.NameSpace != newRs.MetaData.NameSpace {
		...
		podsListTxt := controller.GetObject(config.ObjPod, newRs.MetaData.NameSpace, "")
		if podsListTxt == "" {
		...
		if len(targets) > newRs.Spec.Replicas {
			...
			for _, pod := range targets[newRs.Spec.Replicas:] {
				err := controller.DeleteObject(config.ObjPod, pod.MetaData.NameSpace, pod.MetaData.Name)
				if err != nil {
			...
			for i := 0; i < ops; i++ {
				err = controller.CreateObject(config.ObjPod, newRs.MetaData.NameSpace, pod)
				if err != nil {
					return err
				}
				newRs.Status.RealReplicas++
